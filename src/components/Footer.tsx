"use client";

import React, { useState, useEffect } from "react";
import { motion } from "framer-motion";
import { Mail, ArrowUpRight, FileText } from "lucide-react";

const LinkedinIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" className="w-3.5 h-3.5">
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 01-2.063-2.065 2.064 2.064 0 112.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
  </svg>
);

interface FooterProps {
  onOpenResume: () => void;
}

const GithubIcon = () => (
  <svg viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4">
    <path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12" />
  </svg>
);

const NAV_LINKS = [
  { label: "About", href: "#about" },
  { label: "Experience", href: "#experience" },
  { label: "Projects", href: "#projects" },
  { label: "Skills", href: "#skills" },
  { label: "Contact", href: "#contact" },
];

export default function Footer({ onOpenResume }: FooterProps) {
  const [year, setYear] = useState(2025);
  const [visitors, setVisitors] = useState("");

  useEffect(() => {
    setYear(new Date().getFullYear());
    setVisitors((Math.floor(Math.random() * 800) + 400).toLocaleString("en-US"));
  }, []);

  const scrollTo = (href: string) => {
    const el = document.querySelector(href);
    if (el) window.scrollTo({ top: (el as HTMLElement).offsetTop - 80, behavior: "smooth" });
  };

  return (
    <footer className="relative border-t border-white/[0.05] no-print">
      <div className="max-w-6xl mx-auto px-6 md:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mb-12">
          {/* Brand */}
          <div className="space-y-4">
            <div className="flex items-center gap-2.5">
              <div className="h-8 w-8 rounded-lg bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center">
                <span className="text-emerald-500 text-sm font-bold">S</span>
              </div>
              <span className="text-sm font-semibold text-white">Sai Sankeerth</span>
            </div>
            <p className="text-xs text-zinc-600 leading-relaxed max-w-xs">
              AI Engineer & Backend Developer building intelligent systems,
              scalable APIs, and automated pipelines.
            </p>
            <div className="flex items-center gap-2">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 pulse-available" />
              <span className="text-[10px] text-zinc-600">Available for opportunities</span>
            </div>
          </div>

          {/* Navigation */}
          <div className="space-y-4">
            <p className="text-xs font-semibold tracking-widest text-zinc-600 uppercase">
              Navigate
            </p>
            <nav className="space-y-2.5">
              {NAV_LINKS.map((link) => (
                <button
                  key={link.href}
                  onClick={() => scrollTo(link.href)}
                  className="block text-sm text-zinc-500 hover:text-white transition-colors cursor-pointer text-left"
                >
                  {link.label}
                </button>
              ))}
              <button
                onClick={onOpenResume}
                className="flex items-center gap-1.5 text-sm text-zinc-500 hover:text-white transition-colors cursor-pointer"
              >
                <FileText size={12} />
                Resume
              </button>
            </nav>
          </div>

          {/* Connect */}
          <div className="space-y-4">
            <p className="text-xs font-semibold tracking-widest text-zinc-600 uppercase">
              Connect
            </p>
            <div className="space-y-3">
              <a
                href="mailto:anchurusaisankeerth@gmail.com"
                className="flex items-center gap-2.5 text-sm text-zinc-500 hover:text-white transition-colors group"
              >
                <Mail size={13} />
                <span className="truncate">anchurusaisankeerth@gmail.com</span>
                <ArrowUpRight size={10} className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
              </a>
              <a
                href="https://linkedin.com/in/saisankeerthanchuru"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2.5 text-sm text-zinc-500 hover:text-white transition-colors group"
              >
                <LinkedinIcon />
                saisankeerthanchuru
                <ArrowUpRight size={10} className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
              </a>
              <a
                href="https://github.com/Sankeerth2005"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-2.5 text-sm text-zinc-500 hover:text-white transition-colors group"
              >
                <GithubIcon />
                Sankeerth2005
                <ArrowUpRight size={10} className="shrink-0 opacity-0 group-hover:opacity-100 transition-opacity" />
              </a>
            </div>
          </div>
        </div>

        {/* Bottom bar */}
        <div className="section-divider mb-6" />
        <div className="flex flex-wrap items-center justify-between gap-4">
          <p className="text-xs text-zinc-700">
            © {year} Sai Sankeerth Anchuru. Built with Next.js & Framer Motion.
          </p>
          <div className="flex items-center gap-1.5 text-[10px] text-zinc-700">
            <span className="h-1.5 w-1.5 rounded-full bg-zinc-700" />
            {visitors} visitors
          </div>
        </div>
      </div>
    </footer>
  );
}
